...
<body>
		...
		<div class="col-lg-12">
			<div class="panel panel-default wk-panel wk-table-tools-panel">
				<div class="panel-body">
					<form action="<%=basePath%>searchStudentByName.html" method="POST">
						<div class="form-inline">
							<div class="form-group">
								<label for="filePath" class="control-label wk-filed-label">搜索学生姓名:
								</label>
								<div class="input-group">
									<input required="required" name="studentName" type="text"
					...
					<button id="updateBtn" type="button"
						class="btn btn-default wk-tool-btn">编辑</button>
					<c:choose>
						<c:when test="${pages.currentPage<pages.totalPage}">
							<button id="nextBtn" type="button"
								class="btn btn-default wk-tool-btn">下一页</button>
						</c:when>
					</c:choose>
					<c:choose>
						<c:when test="${pages.currentPage>1}">
							<button id="preBtn" type="button"
								class="btn btn-default wk-tool-btn">上一页</button>
						</c:when>
					</c:choose>
				</div>
			</div>
	...
	<script>
		...
		$("#nextBtn").on("click", function() {
			location.href = "<%=basePath%>searchStudent.html?p=${pages.currentPage+1}";
		});
